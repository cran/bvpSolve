...
Dmeasel <- "
  ...
  df(3,3) = -1.d0/vv
"
cDmeasel <- compile.jacfunc(Dmeasel, header = "  DOUBLE PRECISION :: vv, mu, lam, bet \n  DOUBLE PRECISION, PARAMETER :: pi = 3.141592653589793116D0")
...
"  
cDbound <- compile.jacbound(dbound)
...
print(system.time(
  sol <- bvptwp(fun = cMeasel, bound = cBound, 
    xguess = x, yguess = yguess,
...
print(system.time(
  sol2 <- bvptwp(fun = cMeasel, bound = cBound, jacfunc = cDmeasel, jacbound = cDbound,
    xguess = x, yguess = yguess,
    x=x, leftbc = 3, rpar = c(v, mu, lam), ncomp = 6, 
    nmax = 100000, atol = 1e-8)
))
